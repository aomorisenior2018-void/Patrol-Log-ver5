...
import { FormInput, Toggle, SwitchRow, Card } from './components/FormComponents';
...
const App = () => {
  ...
  useEffect(() => {
    if (currentRecord) {
      ...
      // Merge currentRecord into INITIAL_RECORD_STATE to ensure new fields (deviceStatus etc.) are present even for old records
      setFormData({
        ...
        {(loadingData || !location) ? (
          ...
          <>
            ...
            <form onSubmit={handleSave} className="p-4 space-y-6 pb-24 animate-in fade-in duration-300">
              ...
              {/* Photo Equipment Status (New) */}
              <Card title="機器状態">
                <SwitchRow 
                  label="機器動作確認 (正常)" 
                  checked={formData.deviceStatus === '正常'} 
                  onChange={(c) => updateForm('deviceStatus', c ? '正常' : '異常')} 
                />
                <SwitchRow 
                  label="機器故障 (あり)" 
                  checked={formData.deviceError === 'あり'} 
                  onChange={(c) => updateForm('deviceError', c ? 'あり' : 'なし')} 
                />
                <SwitchRow 
                  label="観測障害 (あり)" 
                  checked={formData.observationError === 'あり'} 
                  onChange={(c) => updateForm('observationError', c ? 'あり' : 'なし')} 
                />
              </Card>
